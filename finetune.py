...
def main():
    ...
    save_root_dir = "./runs/finetune/" + run_id +"/" + dataset_name + '/' + MODEL
    ...
    for lrf in LEARNING_RATE:

        print(f"LR changed to {lrf}")
        run_id = time.strftime("%Y-%m-%d_%H-%M-%S")
        save_dir = save_root_dir + '/' + run_id
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=lrf)
